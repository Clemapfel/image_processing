...
namespace crisp
{
    ...
    class ImageRegion
    {
        ...
        public:
            ...
            const std::vector<Vector2ui>& compute_boundary();

            // @complexity O(k*k) where k number of boundary points
            std::vector<float> compute_farthest_point_signature() const;

            // @complexity amortized o(k) where k number of boundary points
            std::vector<float> compute_radial_distance_signature() const;

            // @complexity amortized o(k) where k number of boundary points
            std::vector<std::complex<float>> compute_complex_coordinate_signature() const;
        ...
        //private:
            ...
            std::vector<float> _farthest_distance_signature,
                               _radial_distance_signature;

            std::vector<std::complex<float>> _complex_signature;
    ...
    template<typename Image_t>
    std::vector<float> ImageRegion<Image_t>::compute_farthest_point_signature() const
    {
    ...
    template<typename Image_t>
    std::vector<float> ImageRegion<Image_t>::compute_radial_distance_signature() const
    {
        auto value = [&](Vector2ui a)
        {
            int a_x = a.x();
    ...
    template<typename Image_t>
    std::vector<std::complex<float>> ImageRegion<Image_t>::compute_complex_coordinate_signature() const
    {
