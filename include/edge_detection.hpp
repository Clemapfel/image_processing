...
namespace crisp::EdgeDetection
{
    ...
    inline BinaryImage threshold_gradient(const GrayScaleImage& original, float threshold = MEASURE_THRESHOLD)
    {
        auto kernel_x = GrayScaleFilter::simple_gradient_x();
        auto kernel_y = GrayScaleFilter::simple_gradient_y();
        ...
        for (long x = 0; x < original.get_size().x; ++x)
        {
            for (long y = 0; y < original.get_size().y; ++y)
            {
                ...
                for (long s = 0; s < kernel_x.rows(); ++s)
                {
                    ...
                    {
                        auto first = kernel_x(s, t);
                        auto two = original.get_pixel_or_padding(x + s, y + t);
                        x_sum += first * two;
                    }
                }
                ...
                if (should_measure)
                {
                    ...
                    max = std::max(max, magnitude_img(x, y));
                }
                else
    ...
    inline BinaryImage threshold_sobel(const GrayScaleImage& original, float threshold = MEASURE_THRESHOLD)
    {
        auto kernel_x = GrayScaleFilter::sobel_x();
    ...
    inline BinaryImage canny(
            const GrayScaleImage& original,
            float lower_threshold = MEASURE_THRESHOLD,
            float upper_threshold = MEASURE_THRESHOLD)
    {
        ...
        for (long x = 0; x < original.get_size().x; ++x)
        {
            for (long y = 0; y < original.get_size().y; ++y)
            {
                ...
                min = std::min(magnitude_img(x, y), min);
                max = std::max(magnitude_img(x, y), max);
                total_sum += magnitude_img(x, y);
            }
        }
        ...
        for (long x = 0; x < original.get_size().x; ++x)
        {
            for (long y = 0; y < original.get_size().y; ++y)
            {
                ...
                for (int i = -1; i <= 1; i++)
                    for (int j = -1; j <= 1; j++)
                        neighborhood(i + 1, j + 1) = magnitude_img.get_pixel_or_padding(x + i, y + j);
        ...
        for (long x = 0; x < out.get_size().x; ++x)
            for (long y = 0; y < out.get_size().y; ++y)
            {
                ...
                for (int i = -2; i <= 2; i++)
                    for (int j = -2; j <= 2; j++)
                    {
                        if (out.get_pixel_or_padding(x + i, y + j) == true)
                        {
